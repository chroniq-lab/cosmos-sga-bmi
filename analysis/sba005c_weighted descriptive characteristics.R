...
analytic_svy <- readRDS(paste0(path_sga_bmi_folder,"/working/sba002/analytic sample with ipw.RDS")) %>% 
  mutate(age_category = case_when(age < 40 ~ "18 to 39",
                                  age < 65 ~ "41 to 64",
                                  age < 75 ~ "65 to 74",
                                  TRUE ~ "75 and older")) %>% 
  mutate(SviOverallPctlRankByZip2020_X = SviOverallPctlRankByZip2020_X*100) %>% 
  survey::svydesign(data=.,~1,weights=~sipw)
...
(sba005c_descriptives <- analytic_svy  %>% 
    tbl_svysummary(by=exposure_binary,
    ...
    add_overall()) %>%
  as_gt() %>%
  gt::gtsave(filename = "analysis/sba005c_weighted descriptive characteristics.html")
