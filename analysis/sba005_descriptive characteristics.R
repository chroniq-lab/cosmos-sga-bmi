...
earliest_analytic <- readRDS(paste0(path_sga_bmi_folder,"/working/sba002/analytic sample with ipw.RDS"))  %>% 
...
(sba005_descriptives <- earliest_analytic %>% 
    ...
    add_overall()) %>%
  as_gt() %>%
  gt::gtsave(filename = "analysis/sba005_descriptive characteristics.html")
