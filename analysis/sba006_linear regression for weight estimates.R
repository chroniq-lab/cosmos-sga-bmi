...
list(
  ...
  saveRDS(.,paste0(path_sga_bmi_folder,"/working/sba006/geeglm linear regression for weight output.RDS"))
...
coefs_model <- bind_rows(
  ...
  tidy_save_geeglm(sba006_output$p0_output[[1]],exponentiate = FALSE) %>% 
    mutate(outcome = sba006_output$p0_output$outcome,
           model = sba006_output$p0_output$model),
  
  tidy_save_geeglm(sba006_output$p1_output[[1]],exponentiate = FALSE) %>% 
    mutate(outcome = sba006_output$p1_output$outcome,
           model = sba006_output$p1_output$model),
  
  tidy_save_geeglm(sba006_output$p2_output[[1]],exponentiate = FALSE) %>% 
    mutate(outcome = sba006_output$p2_output$outcome,
           model = sba006_output$p2_output$model),
  
  tidy_save_geeglm(sba006_output$p3_output[[1]],exponentiate = FALSE) %>%
    mutate(outcome = sba006_output$p3_output$outcome,
           model = sba006_output$p3_output$model),
  
  tidy_save_geeglm(sba006_output$p4_output[[1]],exponentiate = FALSE) %>%
    mutate(outcome = sba006_output$p4_output$outcome,
           model = sba006_output$p4_output$model),
  
  tidy_save_geeglm(sba006_output$p5_output[[1]],exponentiate = FALSE) %>%
    mutate(outcome = sba006_output$p5_output$outcome,
           model = sba006_output$p5_output$model),
  
  tidy_save_geeglm(sba006_output$p6_output[[1]],exponentiate = FALSE) %>%
    mutate(outcome = sba006_output$p6_output$outcome,
           model = sba006_output$p6_output$model),
  
  tidy_save_geeglm(sba006_output$p7_output[[1]],exponentiate = FALSE) %>%
    mutate(outcome = sba006_output$p7_output$outcome,
           model = sba006_output$p7_output$model)
  
)

coefs_model %>% 
  write_csv("analysis/sba006_coefficients of linear regression for weight.csv")
