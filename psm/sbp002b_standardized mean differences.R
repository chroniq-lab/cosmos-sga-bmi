...
analytic_df <- readRDS(paste0(path_sga_bmi_folder,"/working/sbp002/analytic sample with matchit.RDS"))
summary_matchit <- readRDS(paste0(path_sga_bmi_folder,"/working/sbp002/summary of matchit.RDS")) %>% 
  .$sum.matched %>% 
  data.frame() %>% 
...
write_csv(smd,file="psm/sbp002b_standardized mean differences.csv")
