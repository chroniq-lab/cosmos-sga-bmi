...
saveRDS(s,paste0(path_sga_bmi_folder,"/working/sbp002/summary of matchit.RDS"))

readRDS(paste0(path_sga_bmi_folder,"/working/sbp002/summary of matchit.RDS")) %>% 
  .$sum.matched %>% 
  data.frame() %>% 
  mutate(term = rownames(.)) %>% 
  write_csv(.,"psm/sbp002_summary of matchit matched.csv")
...
readRDS(paste0(path_sga_bmi_folder,"/working/sbp002/summary of matchit.RDS")) %>% 
  ...
  data.frame() %>% 
  mutate(term = rownames(.)) %>% 
  write_csv(.,"psm/sbp002_summary of matchit unmatched.csv")

matched_data = match.data(m.out)

ltfu_Bmi_equation <- paste0("Bmi_available ~ exposure_binary + AAPMonthYear_BodyMassIndex",covariates)
ltfu_Bmi_model <- glm(ltfu_Bmi_equation,data=matched_data,family = binomial())
matched_data$prob_Bmi_followup = predict(ltfu_Bmi_model,newdata=matched_data,type="response")

analytic_df = matched_data %>% 
  mutate(mean_e = mean(exposure_binary)) %>% 
  group_by(type_sga) %>% 
  mutate(mean_e_sga = mean(exposure_binary)) %>% 
  ungroup() %>% 
  mutate(
         # sipw = case_when(exposure_binary == 1 ~ mean_e/prob_exposed,
...
saveRDS(analytic_df,paste0(path_sga_bmi_folder,"/working/sbp002/analytic sample with matchit.RDS"))
write_parquet(analytic_df,paste0(path_sga_bmi_folder,"/working/sbp002/analytic sample with matchit.parquet"))
