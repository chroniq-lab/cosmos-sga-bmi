...
earliest_analytic <- readRDS(paste0(path_sga_bmi_folder,"/working/sbp002/analytic sample with matchit.RDS"))  %>% 
  group_by(PatientDurableKey) %>% 
  dplyr::filter(AAPMonthYearKey == min(AAPMonthYearKey)) %>% 
  ungroup() %>% 
...
(sbp005_descriptives <- earliest_analytic %>% 
    ...
    add_overall()) %>%
  as_gt() %>%
  gt::gtsave(filename = "psm/sbp005_descriptive characteristics.html")
