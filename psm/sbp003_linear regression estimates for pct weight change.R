...
list(
  ...
  saveRDS(.,paste0(path_sga_bmi_folder,"/working/sbp003/geeglm linear regression output.RDS"))
...
coefs_model <- bind_rows(
  ...
  tidy_save_geeglm(sbp003_output$m0_output[[1]],exponentiate = FALSE) %>% 
    mutate(outcome = sbp003_output$m0_output$outcome,
           model = sbp003_output$m0_output$model),
  
  tidy_save_geeglm(sbp003_output$m1_output[[1]],exponentiate = FALSE) %>% 
    mutate(outcome = sbp003_output$m1_output$outcome,
           model = sbp003_output$m1_output$model),
  
  tidy_save_geeglm(sbp003_output$m2_output[[1]],exponentiate = FALSE) %>% 
    mutate(outcome = sbp003_output$m2_output$outcome,
           model = sbp003_output$m2_output$model),
  
  tidy_save_geeglm(sbp003_output$m3_output[[1]],exponentiate = FALSE) %>%
    mutate(outcome = sbp003_output$m3_output$outcome,
           model = sbp003_output$m3_output$model),
  
  tidy_save_geeglm(sbp003_output$m4_output[[1]],exponentiate = FALSE) %>%
    mutate(outcome = sbp003_output$m4_output$outcome,
           model = sbp003_output$m4_output$model),
  
  tidy_save_geeglm(sbp003_output$m5_output[[1]],exponentiate = FALSE) %>%
    mutate(outcome = sbp003_output$m5_output$outcome,
           model = sbp003_output$m5_output$model),
  
  
  tidy_save_geeglm(sbp003_output$m6_output[[1]],exponentiate = FALSE) %>%
    mutate(outcome = sbp003_output$m6_output$outcome,
           model = sbp003_output$m6_output$model),
  
  
  tidy_save_geeglm(sbp003_output$m7_output[[1]],exponentiate = FALSE) %>%
    mutate(outcome = sbp003_output$m7_output$outcome,
           model = sbp003_output$m7_output$model)
  
)

coefs_model %>% 
  write_csv("psm/sbp003_coefficients of linear regression for pct weight change.csv")
