...
analytic <- readRDS(paste0(path_sga_bmi_folder,"/working/sbp002/analytic sample with matchit.RDS"))  %>% 
  mutate(semaglutide = case_when(str_detect(WLgenericnames,"semaglutide") ~ 1,
                                 TRUE ~ 0),
         tirzepatide = case_when(str_detect(WLgenericnames,"tirzepatide") ~ 1,
                                 TRUE ~ 0)) %>% 
  mutate(bmi_category_ob = case_when(AAPMonthYear_BodyMassIndex < 30 ~ 1,
                                     AAPMonthYear_BodyMassIndex < 35 ~ 2,
                                     AAPMonthYear_BodyMassIndex < 40 ~ 3,
                                     AAPMonthYear_BodyMassIndex >= 40 ~ 4,
                                     TRUE ~ NA_real_))


table(analytic$semaglutide) %>% prop.table() 
table(analytic$tirzepatide) %>% prop.table() 


# Table generation -------
library(gtsummary)

(sbp005b_descriptives <- analytic %>% 
    mutate(age_category = case_when(age < 40 ~ "18 to 39",
                                    age < 65 ~ "41 to 64",
                                    age < 75 ~ "65 to 74",
                                    TRUE ~ "75 and older")) %>% 
    ...
    tbl_summary(by=exposure_binary,
                ...
                type = list(n_type_sga ~ "continuous2",
                            ...
                            semaglutide ~ "dichotomous", tirzepatide ~ "dichotomous",
    ...
    add_overall()) %>%
  as_gt() %>%
  gt::gtsave(filename = "psm/sbp005b_unweighted descriptive characteristics.html")
