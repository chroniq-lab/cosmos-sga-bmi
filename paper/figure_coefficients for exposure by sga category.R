...
main_coefs <- bind_rows(
  read_csv("analysis/sba003_coefficients of linear regression for pct weight change.csv") %>% 
    mutate(coef = estimate*100,
           lci = lci*100,
           uci = uci*100),
  read_csv("analysis/sba006_coefficients of linear regression for weight.csv") %>% mutate(coef=estimate,outcome = "Weight_after"),
  read_csv("analysis/sba004_coefficients of binomial regression.csv") ) %>% 
...
contrasts <- read_csv("analysis/sba007_contrasts for sga.csv") %>% 
...
write_csv(.,"paper/table_coefficients for exposure by sga categories.csv")
...
ggarrange(fig_A,
          ...
          legend="bottom",
          common.legend = TRUE) %>% 
  ggsave(.,filename=paste0(path_sga_bmi_folder,"/figures/coefficients for exposure by sga categories.jpg"),width=12,height = 5)
